...
function reservarArmario() {
  ...
  armarioSorteado.horarioReserva = Date.now(); //Including date on the selected locker
  ...
  armarioEmprestado.horarioReserva = armarioSorteado.horarioReserva; //Adicionando horario reserva no armario reservado
  
  const d = new Date();
  let day = d.getDate() + 1;
  let hour = d.getHours();
  armarioEmprestado.horarioRetorno = armarioSorteado.horarioRetorno = hour;
  armarioEmprestado.dataRetorno = armarioSorteado.dataRetorno = day;
  ...
  document.getElementById("resultado").innerText = `Olá, ${usuario.nome}! O armário ${armarioSorteado.id} foi reservado com sucesso!` + `A entrega das chaves deverá ser realizada até o dia ${day} às ${hour} horas`;
